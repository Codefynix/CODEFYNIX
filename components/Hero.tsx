...
export default function Hero({
  ...
  const renderWords = (title: string) =>
    title.split(" ").map((word, idx) => (
      <span key={`${word}-${idx}`} className="word-wrap">
        <span className="hero-word word-inner">{word}</span>
      </span>
    ));
  ...
  return (
    <section
      ref={heroRef}
      id="home"
      className="relative flex min-h-screen items-center overflow-hidden bg-[#0D0D0D] pt-24"
    >
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-12 px-4 sm:px-6 lg:px-8">
        <div className="hero-content hero-content-wrap relative z-10 max-w-4xl">
          <p className="hero-tag hero-badge font-accent text-sm uppercase tracking-[0.25em] text-[#5BBF1A]">
            {current.tag}
          </p>
          {active === 0 ? (
            <h1 className="hero-h1 mt-5 font-heading text-4xl font-bold leading-tight text-white md:text-6xl lg:text-7xl">
              {renderWords(current.title)}
            </h1>
          ) : (
          ...
          <p className="hero-sub hero-sub-text mt-5 max-w-2xl font-body text-lg text-zinc-300">
            {current.sub}
          </p>
