  ...
  return (
    <section id="blog-home" className="section-wrap bg-white">
      <div className="section-inner">
        ...
        <h2 className="section-title word-reveal-heading">News & Updates from Codefynix</h2>

        <div className="mt-9 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {blogPosts.slice(0, 6).map((post) => (
            <article key={post.slug} className="blog-card reveal overflow-hidden rounded-2xl border border-zinc-200 bg-white" data-hover>
              ...
              <div className="p-5">
                ...
                <h3 className="mt-3 font-heading text-xl font-semibold text-[#111111] transition hover:text-[#5BBF1A]">
                  <Link href={`/blog/${post.slug}`} data-hover>{post.title}</Link>
                </h3>
                <p className="mt-3 font-body text-sm text-[#666666]">{post.excerpt}</p>
                <Link href={`/blog/${post.slug}`} data-hover className="mt-4 inline-flex text-sm font-semibold text-[#5BBF1A]">
                  Read More →
                </Link>
              </div>
            </article>
