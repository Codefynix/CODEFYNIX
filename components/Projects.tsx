  ...
  return (
    <section id="projects-home" className="section-wrap bg-[#F5F5F5]">
      <div className="section-inner">
        ...
        <h2 className="section-title">Explore Our Recent Work</h2>
        ...
        <div ref={gridRef} className="mt-8 -m-[10px]">
          {items.map((project) => (
            <article
              key={project.slug}
              className={`project-item ${project.category.replace(/\s+/g, "-")} w-full p-[10px] sm:w-1/2 lg:w-1/3`}
            >
              <div className="group relative overflow-hidden rounded-2xl">
                ...
                <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/80 via-black/20 to-transparent p-5">
                  ...
                  <Link href={`/projects/${project.slug}`} className="mt-2 inline-flex w-fit text-sm font-semibold text-[#7DD63A]">
                    View →
                  </Link>
                </div>
              </div>
