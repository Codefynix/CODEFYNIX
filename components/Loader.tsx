...
export default function Loader({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!root || !wrap || !canvas || !glow || !scan || !progress || !ring1 || !ring2 || !ring3 || !dot1 || !dot2 || !dot3) return;
    ...
    const gsapCtx = gsap.context(() => {
      ...
      tl.to(wrap, { opacity: 1, scale: 1, duration: 0.7, ease: "back.out(1.5)" })
        .to(glow, { opacity: 1, duration: 0.5, ease: "power2.out" }, "-=0.4");
      ...
      gsap.to(orbitState1, {
        angle: 360, duration: 3, ease: "none", repeat: -1,
        onUpdate() {
          const p = orbitXY(orbitState1.angle, 108);
          dot1.setAttribute("cx", String(p.x));
          dot1.setAttribute("cy", String(p.y));
        },
      });
      gsap.to(orbitState2, {
        angle: 540, duration: 4.5, ease: "none", repeat: -1,
        onUpdate() {
          const p = orbitXY(orbitState2.angle, 108);
          dot2.setAttribute("cx", String(p.x));
          dot2.setAttribute("cy", String(p.y));
        },
      });
      gsap.to(orbitState3, {
        angle: 450, duration: 2.2, ease: "none", repeat: -1,
        onUpdate() {
          const p = orbitXY(orbitState3.angle, 72);
          dot3.setAttribute("cx", String(p.x));
          dot3.setAttribute("cy", String(p.y));
        },
      });

      // Scan line sweep
      gsap.to(scan, { opacity: 1, duration: 0.3, delay: 0.6 });
      gsap.to(scan, { top: "20%", duration: 1.2, delay: 0.6, ease: "power1.inOut", yoyo: true, repeat: 2 });
      gsap.to(scan, { opacity: 0, duration: 0.3, delay: 3.0 });

      // Progress bar
      gsap.to(progress, { width: "100%", duration: 2.8, delay: 0.5, ease: "power1.inOut" });
      ...
      <div ref={wrapRef} className="relative flex items-center justify-center">
        ...
        <svg className="absolute" width="260" height="260" viewBox="0 0 260 260">
          <circle
            ref={ring1Ref}
            cx="130" cy="130" r="90"
            fill="none" stroke="#5BBF1A"
            strokeWidth="1.5" strokeDasharray="40 520"
          ...
          <circle
            ...
            cx="130" cy="130" r="108"
            fill="none" stroke="#5BBF1A"
            strokeWidth="1" strokeDasharray="20 620"
          ...
          <circle
            ...
            cx="130" cy="130" r="72"
            fill="none" stroke="#5BBF1A"
            strokeWidth="2" strokeDasharray="60 400"
        ...
        <div
          ...
          style={{
            background: "radial-gradient(circle, rgba(91,191,26,0.35) 0%, transparent 70%)",
            filter: "blur(16px)",
          }}
        />
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-40">
        ...
        >
          <div
            ...
            style={{ width: 0, background: "#5BBF1A" }}
          />
        </div>
        <p
          className="text-center mt-2 text-[10px] font-mono tracking-[3px]"
          style={{ color: "rgba(91,191,26,0.6)" }}
        >
          LOADING
        </p>
      </div>
